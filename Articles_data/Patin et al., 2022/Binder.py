...
def MFT(sample, lamp):    
    '''
    Function that plot the results from the light box experiment (project 2.2)

    Inputs
    ====================================================
    PO: a string
    Paint-out number or sample number
    
    position: a string
    Measurement position on the PO number ('wbg1', 'wbg2', 'bbg1', 'bbg2' or 'none'). 
    
    device: a string
    Spectrometer that was used to measure the reflectance spectrum of the sample ('CM-2600d' or 'OceanOptics')
    
    ratio: a string
    Indicate the pigments ratio 
    
    dark: a string
    Id of the PO left in the dark area of the light box
    
    dir_path: a string 
    Path of the directory where the final graph should be saved

    Returns
    ====================================================
    It returns a single png file - containing 6 subplots:
    ...
    with out6:
        ...
        def f(ill_slider,t_slider,hpd):
            ...
            if d <= max_dose:
                ...
                ax8.annotate('Estimated colour',(0.77, 0.05),fontsize = 20)
